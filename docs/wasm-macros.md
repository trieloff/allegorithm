...
One debt is recorded in daylight: `scripts/projector.ts` still interprets films in TypeScript. The design that retires it is the same trick told twice – a `(film …)` macro library expands the declarative film into a module whose filters are explicitly imported wasm modules speaking the byte protocol, and the host shrinks to a capability shim (Chromium for Kokoro and WebGPU, ffmpeg.wasm for the cut) with no `.hma` knowledge at all. The compiler walked this road first; the projector follows it.
