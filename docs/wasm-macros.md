...
5. **Stage 4** – the expander itself in Wasm, GC AST as specified in § 2, meta-Lisp interpreted; then Binaryen IR as an alternate lowering, and the browser as an expansion host – a sandboxed expander that will not run in a `<script>` tag is leaving its best argument unused. Acceptance: it expands its own source, and stage 0 and stage 4 produce bit-identical output on the golden suite.
