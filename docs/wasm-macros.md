...
6. **Stage 5** – the GC AST of § 2 in earnest, once the assembler learns rec groups and `br_on_cast`; Binaryen IR as an alternate lowering; the browser as an expansion host – a sandboxed expander that will not run in a `<script>` tag is leaving its best argument unused.
