...
describe.skipIf(!runtime)('expand-gc.hma — the expander on the GC heap', () => {
  ...
  it('matches stage 0 on the assembler and the Clojure corpus', () => {
    for (const files of [
      ['src/hotglue/prelude.hma', 'src/hotglue/as.hma'],
      ['src/hotglue/clj.hma', 'examples/collatz.hma'],
    ]) {
  ...
  it('expands its own source to the text it is running as', () => {
    const source = src('src/hotglue/prelude.hma', 'src/hotglue/expand-gc.hma');
  ...
  it('runs on the heap it can be assembled from: the GC loop closes', () => {
    ...
    const again = pipe(gcWasm, src('src/hotglue/prelude.hma', 'src/hotglue/expand-gc.hma'));
